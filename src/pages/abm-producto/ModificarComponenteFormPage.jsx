...
import { ModificacionExitosaModal } from '../../components/modals/abm-producto/ModificacionExitosaModal';
import { ImagePreviewModal } from '../../components/modals/abm-producto/ImagePreviewModal';

export const ModificarComponenteFormPage = () => {
  ...
  useEffect(() => {
    ...
    const handlePopState = (event) => {
      ...
      navigate('/admin/componentes/modificar', { replace: true });
    };
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      productoService.obtenerProducto(codigo)
    ])
    .then(([categoriasData, productoData]) => {
      setCategorias(categoriasData);
      
      setFormData({
        nombreTecnico: productoData.nombreTecnico || '',
        imagenUrl: productoData.imagenUrl || '',
        categoriaCodigo: productoData.categoriaCodigo || ''
      });
      
      if (productoData.especificaciones) {
        const specsArray = Object.entries(productoData.especificaciones).map(([k, v]) => ({ key: k, value: String(v) }));
        setEspecificaciones(specsArray);
      }
    })
    .catch(err => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      await productoService.modificarProducto(codigo, payload);
      setShowExitosa(true);
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#CBD5E1', fontFamily: 'Nunito', padding: '2rem' }}>
      ...
      <div style={{
        ...
        <div style={{ width: '100%', display: 'flex', alignItems: 'center', position: 'relative' }}>
          <button 
            onClick={() => navigate('/admin/componentes/modificar', { replace: true })}
            style={{
          ...
          <div style={{ width: '100%', textAlign: 'center' }}>
            <h1 style={{ margin: 0, color: '#334155', fontSize: '1.5rem', fontWeight: 800 }}>MendoHard - Modificar un Componente de Hardware</h1>
            <h2 style={{ margin: 0, color: '#334155', fontSize: '1.2rem', fontWeight: 600 }}>Ingrese los datos correspondientes</h2>
          </div>
        </div>
      ...
      {/* Modals */}
      {showExitosa && <ModificacionExitosaModal onClose={handleSuccessClose} onAccept={handleSuccessClose} />}

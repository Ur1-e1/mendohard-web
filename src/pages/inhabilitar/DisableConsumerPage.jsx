...
export const DisableConsumerPage = () => {
  ...
  const fetchConsumidores = async () => {
    try {
      setLoading(true);
      const data = await inhabilitarService.getConsumidoresActivos();
  ...
  const handleSeleccionar = async (uCodigo) => {
    try {
      await inhabilitarService.inhabilitarConsumidor(uCodigo);
  ...
  const handleError = (error) => {
    if (error.response) {
      ...
      if (status === 400 && errorCode === 'DATA_INCONSISTENCY') {
        ...
        setIsServerErrorModalOpen(true); // Fallback genérico
      }
    } else {
      setIsServerErrorModalOpen(true);
    }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          Seleccionar un Consumidor a inhabilitar
        </p>

        {loading ? (
          <p style={{ color: '#334155' }}>Cargando consumidores...</p>
        ) : consumidores.length === 0 ? (
          <p style={{ color: '#334155', fontWeight: 'bold' }}>No hay consumidores activos para inhabilitar.</p>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', width: '100%' }}>
            {consumidores.map((consumidor) => (
              <div key={consumidor.uCodigo} style={{
                border: '1px solid #E2E8F0',
                borderRadius: '8px',
                padding: '1rem',
                display: 'flex',
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', color: '#1E293B', fontWeight: '500' }}>
                  <span>{consumidor.uNombre} {consumidor.uApellido}</span>
                  <span>Apodo: {consumidor.cApodo}</span>
                </div>
                <div style={{ color: '#334155', fontSize: '0.9rem' }}>
                  Email: {consumidor.uEmail}
                </div>
                
                <button
                  onClick={() => handleSeleccionar(consumidor.uCodigo)}
                  style={{
                    marginTop: '0.5rem',
      ...
      <DataInconsistencyModal 
        ...
        // Note: DataInconsistencyModal currently does not accept a message prop and has a hardcoded one.
        // We will just open it as is per requirements.
      />
